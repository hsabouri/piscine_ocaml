...
let ordered = Card.all

let emptyDeck = []

let addToRandom l head = match Random.int 3 with
	| 0 | 1 -> head :: l
	| _ -> l @ [head]

let newDeck unit =
	let rec construct acc remaining = match remaining with
		| head :: tail -> construct (addToRandom acc head) tail
		| [] -> acc
	in
	let rec iteration l i = match i with
		| i when i <= 0 -> l
		| i -> iteration (construct [] l) (i - 1)
	in
	iteration Card.all 4

let toStringList t = List.map (fun x -> Card.toString x) t
let toStringListVerbose t = List.map (fun x -> Card.toStringVerbose x) t
